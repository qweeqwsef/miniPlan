...
class MiniManagerEnv12(gym.Env):
    ...
    def _apply_fixed_forgetting(self, mastery: np.ndarray) -> tuple[np.ndarray, dict]:
        ...
        enhanced_params = {
            'thresholds': {
                'algebra': 4,    # 从6减少到4（更早触发遗忘）
                'geometry': 3,   # 从4减少到3（更早触发遗忘）  
                'statistics': 5  # 从7减少到5（更早触发遗忘）
            },
            'strengths': {
                'algebra': 0.08,     # 从0.036增加到0.08（2.2倍强度）
                'geometry': 0.08,    # 从0.036增加到0.08（2.2倍强度）
                'statistics': 0.08   # 从0.036增加到0.08（2.2倍强度）
            },
            'adjust_factor': 0.7,    # 从0.5增加到0.7（增强影响）
        }
    ...
    def _apply_improved_forgetting(self, mastery: np.ndarray) -> tuple[np.ndarray, dict]:
        ...
        thresholds = {
            'algebra': 4,    # 与FF保持一致
            'geometry': 3,   # 与FF保持一致
            'statistics': 5  # 与FF保持一致
        }
        
        # IF特有参数 - Stability-Aware Threshold Forgetting
        if_params = {
            'base_decay': 0.08,        # 与FF基础强度一致
            'stability_k': 8.0,         # 稳定性计算参数（保持不变）
            'stability_lambda': 0.7,    # 稳定性影响因子（保持不变）
        }
    ...
    def step(self, action: int):
        ...
        resource_fatigue = 0.0  # 高层不直接管理资源疲劳，设为0
        ...
        if self.forgetting_mode in ['fixed_forgetting', 'improved_forgetting']:
            # 只有在有遗忘机制时，Timer高了才扣分
            # 这会逼迫Agent去压低Timer（即轮换复习）
            timer_penalty_coef = 0.5  # 【EXP15修复】提高到0.5，迫使策略必须轮换
...
class FlatMiniEnv12(gym.Env):
    ...
    def _apply_fixed_forgetting(self, mastery: np.ndarray) -> tuple[np.ndarray, dict]:
        ...
        enhanced_params = {
            'thresholds': {
                'algebra': 4, 'geometry': 3, 'statistics': 5  # 与分层环境完全一致
            },
            'strengths': {
                'algebra': 0.08, 'geometry': 0.08, 'statistics': 0.08  # 与分层环境完全一致
            },
            'adjust_factor': 0.7,  # 与分层环境完全一致
        }
    ...
    def _apply_improved_forgetting(self, mastery: np.ndarray) -> tuple[np.ndarray, dict]:
        """修复C: 连续遗忘衰减的改进遗忘机制"""
        thresholds = {'algebra': 4, 'geometry': 3, 'statistics': 5}  # 与分层环境完全一致
        
        if_params = {
            'base_decay': 0.08,  # 与分层环境完全一致
            'stability_k': 8.0,
